...
export const getChatList = async (req, res) => {
    ...
    try {
        ...
        if (userRooms.length === 0) {
            return res.status(200).json({ message: 'No rooms found for the user.' });
        }
        ...
        for (const room of userRooms) {  // Use for...of for cleaner iteration
            const otherUser = room.users.find(id => id.toString() !== userId.toString());

            if (otherUser) { // Check if otherUser exists
                const otherPerson = await Users.findOne(otherUser).select('email phone username');
         ...
         res.status(200).json({ status: "SUCCESS",data: roomsWithOtherUsers });
